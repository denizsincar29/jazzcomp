...
В данной бетаверсии программа не имеет графического интерфейса, поэтому для взаимодействия с пользователем решено минимизировать работу в консоли. Программа возьмёт на вход положенный рядом с ним файл input.txt с гармонической сеткой и сгенерирует output.wav файл с аккомпанементом, который будет сохранён в той же папке.
...
- В случайных местах редко вставляются хроматические переходы по гамме, синкопы и синкопированное остенато.
...
todo: после написания полной версии программы.

# 4. Тестирование
## 4.1. Написание Unit-тестов
Юниттесты написаны для всех функций, которые имеют определённый результат. Для каждого модуля написан файл с тестами, именованный test_<имя_модуля>.py. Тесты написаны с использованием библиотеки pytest. Для запуска тестов нужно выполнить команду `uv run pytest` в терминале.
