...
Есть аналогичные программы, такие как [Band-in-a-Box](https://www.pgmusic.com/) и [iReal Pro](https://irealpro.com/), но они не считаются конкурентами, т.к. они могут генерировать аккомпанемент в разных стилях, имеют большой функционал, но при этом стоят денег и генерируют шаблонные аккомпанементы.
